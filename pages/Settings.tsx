...
const NotificationToggle: React.FC = () => {
  ...
  useEffect(() => {
    isPushSubscribed().then(val => { setSubscribed(val); setLoading(false); });
  }, []);
  ...
  const toggle = async () => {
    ...
    if (!newState) {
      ...
      if (!ok) {
        setSubscribed(false);
        setError('Could not enable notifications. Open browser Settings and allow this site.');
      }
    }
  };
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="text-2xl">{subscribed ? (<span style={{display:"inline-flex",alignItems:"center",verticalAlign:"middle"}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg></span>) : (<span style={{display:"inline-flex",alignItems:"center",verticalAlign:"middle"}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M13.73 21a2 2 0 0 1-3.46 0"/><path d="M18.63 13A17.89 17.89 0 0 1 18 8"/><path d="M6.26 6.26A5.86 5.86 0 0 0 6 8c0 7-3 9-3 9h14"/><path d="M18 8a6 6 0 0 0-9.33-4.99"/><line x1="1" y1="1" x2="23" y2="23"/></svg></span>)}</span>
          <div>
            ...
            <p className="text-xs text-slate-500">
              {subscribed
                ? 'You will get alerts 7 days and 1 day before deadlines'
                : 'Enable to get tax deadline alerts'}
            </p>
          </div>
        </div>
        <button
          onClick={toggle}
          disabled={loading}
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors disabled:opacity-50 ${subscribed ? 'bg-cac-green' : 'bg-slate-200'}`}
        >
      ...
      {!subscribed && !error && (
        <p className="text-xs text-slate-400">
          Note: your browser will only ask for permission once. If you previously denied it, go to browser Settings to re-enable.
        </p>
      )}
    </div>
  );
...
const RegenerateButton: React.FC<{ company: Company }> = ({ company }) => {
  ...
  const handleRegenerate = async () => {
    if (!window.confirm(
      'This will add any missing tax obligations based on your current settings. Existing obligations will NOT be deleted. Continue?'
    )) return;
    setLoading(true); setErr(''); setDone(false);
    try {
      const obligations = generateObligations(company);
      for (const ob of obligations) {
        try { await db.addObligation({ ...ob, id: '' }); }
        catch (e: any) {
...
export const SettingsPage: React.FC<SettingsProps> = ({ company, onCompanyUpdate }) => {
  ...
  const isIndividual = form.entityType === EntityType.INDIVIDUAL;
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Card className="space-y-4">
        ...
        <div>
          ...
          <select value={form.entityType} onChange={e => update('entityType', e.target.value as EntityType)}
        ...
        {!isIndividual && (
          <>
            <label className="flex items-center justify-between p-3 bg-slate-50 rounded-xl cursor-pointer hover:bg-slate-100 transition">
              <div>
                <p className="font-semibold text-sm text-slate-800">Has Employees (PAYE)</p>
                <p className="text-xs text-slate-400">PAYE due 10th of each month to State IRS</p>
              </div>
              <input type="checkbox" className="accent-cac-green w-4 h-4"
                checked={form.hasEmployees} onChange={e => update('hasEmployees', e.target.checked)} />
            </label>
            {form.hasEmployees && (
            ...
            <label className="flex items-center justify-between p-3 bg-slate-50 rounded-xl cursor-pointer hover:bg-slate-100 transition">
              <div>
                <p className="font-semibold text-sm text-slate-800">Pays Vendors (WHT)</p>
                <p className="text-xs text-slate-400">5% goods, 10% services. Due 21st monthly to NRS</p>
              </div>
              <input type="checkbox" className="accent-cac-green w-4 h-4"
                checked={form.paysVendors} onChange={e => update('paysVendors', e.target.checked)} />
            </label>
          </>
        )}
        {!isIndividual && (
          <label className="flex items-center justify-between p-3 bg-slate-50 rounded-xl cursor-pointer hover:bg-slate-100 transition">
            <div>
              <p className="font-semibold text-sm text-slate-800">Collects VAT</p>
              <p className="text-xs text-slate-400">NTA 2025: threshold is 100M+ turnover</p>
            </div>
            <input type="checkbox" className="accent-cac-green w-4 h-4"
              checked={form.collectsVat} onChange={e => update('collectsVat', e.target.checked)} />
          </label>
        )}
        {isIndividual && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-4">
            ...
            <div>
              ...
              <select value={form.employmentType || 'employed'}
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5">Estimated Annual Gross Income (NGN)</label>
              <input type="number" value={form.annualIncome || ''}
                onChange={e => update('annualIncome', parseFloat(e.target.value) || undefined)}
                placeholder="e.g. 4800000"
                className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-cac-green/30 text-sm" />
            </div>
          </div>
        )}
      </Card>

      {!isIndividual && form.hasEmployees && (
        <Card className="space-y-4">
          <h2 className="font-bold text-slate-800">Payroll Statutory Contributions</h2>
          {[
            { key: 'hasNSITF',  label: 'NSITF (3+ employees)',            desc: '1% of gross payroll, due 16th monthly' },
            { key: 'hasPension', label: 'Pension (3+ employees)',          desc: '8% employee + 10% employer, due 7 days after payday' },
            { key: 'hasITF',    label: 'ITF (5+ staff or 50M+ turnover)', desc: '1% of annual payroll, due 1 April' },
            { key: 'hasNHF',    label: 'NHF contributions',               desc: '2.5% of employee basic salary, monthly' },
          ].map(({ key, label, desc }) => (
            <label key={key} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl cursor-pointer hover:bg-slate-100 transition">
              ...
              <input type="checkbox" className="accent-cac-green w-4 h-4"
                ...
                onChange={e => update(key as keyof Company, e.target.checked)} />
            </label>
          ))}
        </Card>
      )}

      {!isIndividual && form.cacStatus === 'Registered' && (
        <Card className="space-y-3">
          ...
          <label className="flex items-center justify-between p-3 bg-slate-50 rounded-xl cursor-pointer hover:bg-slate-100 transition">
            <div>
              ...
              <p className="text-xs text-slate-400">File with CAC by 30 June each year. Penalty: 3,000 per day.</p>
            </div>
            <input type="checkbox" className="accent-cac-green w-4 h-4"
              checked={form.cacAnnualReturns !== false}
              onChange={e => update('cacAnnualReturns', e.target.checked)} />
          </label>
        </Card>
      )}

      <Card className="space-y-4">
        <h2 className="font-bold text-slate-800">WhatsApp Reminders</h2>
        <Input label="WhatsApp / Phone Number" type="tel"
        ...
        {form.phone && (
          <label className="flex items-center justify-between p-3 bg-green-50 rounded-xl cursor-pointer hover:bg-green-100 transition border border-green-200">
            <div>
              ...
              <p className="text-xs text-slate-400">Receive deadline alerts on WhatsApp</p>
            </div>
            <input type="checkbox" className="accent-cac-green w-4 h-4"
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-3">
        ...
        <p className="text-xs text-blue-700">Regenerate your tax schedule to add any new obligations.</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-50">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Push notifications 7 days and 1 day before deadlines</p>
        </div>
        <div className="p-6"><NotificationToggle /></div>
      </div>
    </div>
  );
};
